...
- (void)callSetIsHidden:(NSNumber *)mobileAdId
                   call:(FlutterMethodCall *)call
                 result:(FlutterResult)result {
    FLTMobileAd *ad = [FLTMobileAd getAdForId:mobileAdId];
    if (ad == nil) {
    ...
    if (![ad isKindOfClass:FLTBannerAd.class]) {
        result([FlutterError errorWithCode:@"ad_wrong_format" message:nil details:nil]);
        return;
    }
    FLTBannerAd *banner = (FLTBannerAd *) ad;
    NSNumber *isHidden = call.arguments[@"isHidden"];
    [banner setIsHidden:isHidden.boolValue];
    result(nil);
}

- (void)callSetPosition:(NSNumber *)mobileAdId
                   call:(FlutterMethodCall *)call
                 result:(FlutterResult)result {
    FLTMobileAd *ad = [FLTMobileAd getAdForId:mobileAdId];
    if (ad == nil) {
    ...
    [banner setOffset:offset fromAnchor:type];
    result(nil);
}
...
- (void)handleMethodCall:(FlutterMethodCall *)call result:(FlutterResult)result {
  ...
  } else if ([call.method isEqualToString:@"setIsHidden"]) {
    [self callSetIsHidden:mobileAdId call:call result:result];
  } else if ([call.method isEqualToString:@"setPosition"]) {
    [self callSetPosition:mobileAdId call:call result:result];
  } else {
