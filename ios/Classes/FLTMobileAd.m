...
- (void)show {
  ...
  if (@available(ios 11.0, *)) {
    ...
    UILayoutGuide *guide = screen.layoutMarginsGuide;
    _verticalConstraint = [_banner.bottomAnchor
...
- (void)setOffset:(double)anchorOffset fromAnchor:(int)anchorType {
  _anchorType = anchorType;
  _anchorOffset = anchorOffset;
  UIView *screen = [FLTMobileAd rootViewController].view;
  if (@available(ios 11.0, *)) {
    UILayoutGuide *guide = screen.layoutMarginsGuide;
    if (_verticalConstraint != nil) {
      [_verticalConstraint setActive:false];
    }
    _verticalConstraint = [_banner.bottomAnchor
                           constraintEqualToAnchor:anchorType == 0 ? guide.bottomAnchor : guide.topAnchor
                           constant:anchorType == 0 ? (anchorOffset+_banner.frame.size.height) : (_banner.frame.size.height + anchorOffset)];
    [_verticalConstraint setActive:true];
  } else {
    CGFloat x = screen.frame.size.width / 2 - _banner.frame.size.width / 2;
    CGFloat y;
    if (anchorType == 0) {
      y = screen.frame.size.height - _banner.frame.size.height + anchorOffset;
    } else {
      y = anchorOffset;
    }
    _banner.frame = (CGRect){{x, y}, _banner.frame.size};
