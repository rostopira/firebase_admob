...
public class FirebaseAdMobPlugin implements MethodCallHandler {
  ...
  private void callSetIsHidden(int id, MethodCall call, Result result) {
    MobileAd ad = MobileAd.getAdForId(id);
    if (ad == null) {
      result.error("ad_not_loaded", "show failed, the specified ad was not loaded id=" + id, null);
      return;
    }
    if (!(ad instanceof MobileAd.Banner)) {
      result.error("ad_is_not_banner", "failed", null);
      return;
    }
    Boolean isHidden = call.argument("isHidden");
    ((MobileAd.Banner) ad).setIsHidden(isHidden);
    result.success(Boolean.TRUE);
  }

  private void callSetPosition(int id, MethodCall call, Result result) {
    MobileAd ad = MobileAd.getAdForId(id);
    if (ad == null) {
      result.error("ad_not_loaded", "show failed, the specified ad was not loaded id=" + id, null);
      return;
    ...
    if (call.argument("anchorOffset") != null) {
      anchorOffset = Double.parseDouble((String) call.argument("anchorOffset"));
    }
    if (call.argument("anchorType") != null) {
      anchorType = call.argument("anchorType").equals("bottom") ? Gravity.BOTTOM : Gravity.TOP;
    }
    ((MobileAd.Banner) ad).setPosition(anchorType, anchorOffset);
    result.success(Boolean.TRUE);
  }
  ...
  public void onMethodCall(MethodCall call, Result result) {
    ...
    switch (call.method) {
      ...
      case "showAd":
        ...
        break;
      case "setIsHidden":
        callSetIsHidden(id, call, result);
        break;
      case "setPosition":
        callSetPosition(id, call, result);
        break;
